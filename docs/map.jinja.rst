...
We will see a quick setup to configure the ``tool_vim`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_vim`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_vim`` formulas under ``/srv/salt/tool_vim/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_vim/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_vim/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_vim/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_vim/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_vim`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_vim`` formulas under ``/srv/salt/tool_vim/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_vim/parameters/roles
...
We create a configuration for the role ``tool_vim/server`` in ``/srv/salt/tool_vim/parameters/roles/tool_vim/server.yaml``:
...
We create another configuration for the role ``tool_vim/client`` in ``/srv/salt/tool_vim/parameters/roles/tool_vim/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_vim/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_vim/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_vim/parameters/dns:domain/  '/srv/salt/tool_vim/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-vim-as-json:
      test.nop:
        - name: {{ vim | json }}
...
.. code-block:: sls
    ...
    vim-config-file-file-managed:
      file.managed:
        - name: {{ vim.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='vim-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``vim`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ vim['winner'] }}
